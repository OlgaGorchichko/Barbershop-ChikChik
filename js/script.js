const API_URL = 'https://graceful-mica-aletopelta.glitch.me/';
/*
    ...
    window.addEventListener('resize', () => {
        if (activeSlide + 2 > sliderItems.length && 
            document.documentElement.offsetWidth > 560) {
                ...
                sliderItems[activeSlide]?.classsList.add('slider__item-active')
            }
...
const initService = () => {
    ...
    const reserveFieldserService = document.querySelector('.reserve__fieldset_service');
    priceList.textContent = '';
    addPreload(priceList);

    reserveFieldserService.innerHTML = '<legend class="reserve__legend">Услуга</legend>';
    addPreload(reserveFieldserService);
    ...
    fetch(`${API_URL}api`)
        ...
        .then(data => {
            renderService(reserveFieldserService, data);
            removePreload(reserveFieldserService);
        })
};
